...
mFLOCSS は、CSS の記述を層に分類する思考フレームワークである。本仕様は「このスタイルをどの層に書くか」の判断基準・命名規則・ファイル構成を定義する。
...
mFLOCSS の中心は「どの層に、なぜ書くか」の判断基準である。命名規則や MUST ルールはこの判断を保護するために存在し、規則の暗記ではなく判断の再現を目的とする。

アーキテクチャは `@layer` ベースのフラット構成を採用する。`@layer`（CSS Cascading and Inheritance Level 5 [CSS-CASCADE-5]）は、スタイルの優先順位をセレクタ詳細度に依存せず宣言順で制御する CSS 標準機能である。
...
> **注記（Informative）**: §4 MUST NOT [!important の使用制限] の「Utility 層を除く全層」は、project が §3 の標準層構成に加えて宣言する追加層（§3 注記および §4 [外部 CSS の層配置判断] の例外注記が挙げる慣用名 `vendor` 等）を含む。§4「例外: `!important` の許容範囲」類型 2 が本制限の対象外とするのは、取り込んだ外部 CSS の内部実装における `!important` の有無であり、project が当該層に書き足す宣言はこれに含まれない。また、前段の外部生成クラスに対する免除は、そのクラスをセレクタに用いること、外部生成クラスとの組み合わせ、およびそのクラス名が §6 の命名規則に従わないことについてのものであり、project が記述する宣言の内容（`!important` の付与を含む）を免除しない。したがって、追加層に取り込んだ第三者 CSS の `!important` 宣言（類型 2）を打ち消す目的で Utility 層以外の層に自作の `!important` 宣言を記述することは、MUST NOT [!important の使用制限] に違反する。なお、Utility 層の `!important` であっても、層順序によっては第三者 CSS の `!important` を打ち消せない（§4 [外部 CSS の層配置判断] の例外注記を参照）。MUST NOT からの逸脱は、SHOULD からの逸脱と異なり正当な理由による許容の対象ではなく、当該 project は本仕様への準拠を主張できない。それでも逸脱を残す場合、その箇所と理由をコード内コメントまたは設計文書に明示することを推奨する（記録は準拠状態を回復しない）。
...
> **注記（Informative）**: SHOULD に違反してもカスケードは壊れないが、層の判断基準が形骸化する。逸脱時は前掲の注記に従い理由を記録すること。

### バージョニング

バージョンは「メジャー.マイナー.パッチ」の3桁で表記する。

- メジャー: 層の追加・削除・統合。MUST/MUST NOT の追加・削除・変更（他レベルからの昇格・降格を含む）
- マイナー: SHOULD/MAY の追加・削除・レベル間の変更
- パッチ: Informative セクションの変更・誤記修正・Example の改善等、要求レベルに影響しない変更
...
> **注記（Informative）**: 先制宣言で確定した層順序では、通常宣言について後に宣言された層ほど優先されるため、テーブルの下にある層ほど優先度が高い。`!important` 宣言では層順序が逆転する（先に宣言した層が優先される）ため、Utility の最終上書きは宣言順ではなく、§5.8 MUST [!important の付与] と §4 MUST NOT [!important の使用制限] の組で成立する。§4 の例外により Utility より前に宣言された層が `!important` を持つ場合は、層順序の逆転によりその宣言が Utility に優先される。
...
> **注記（Informative）— 原則**: MUST [外部 CSS の層取り込み] が定める「いずれかの層」の選び方は、自作スタイルと同じ基準、すなわち**役割**で判断する。役割をひとつに割り当てられる外部 CSS—UI 部品系ライブラリ（スライダー等、機能を活かしてテーマ調整したいもの）—はその責任層（例: component）に取り込む。テーマ調整は project 層のカスタムプロパティ上書きで行う。機能ライブラリを低位層に置かない理由は、責任層より下位に配置すると自作の foundation 等が当該ライブラリの機能スタイルに優先してしまうためである（`@layer` の優先度は詳細度に勝るため、`:where()` による低詳細度ルールでも当該ライブラリの通常宣言を上書きする。ただしライブラリ側が `!important` を使用している宣言は、重要度が通常宣言に優先するため上書きできない）。責任層に配置することで下位の foundation 等から守られ、意図した箇所のみ project 層で上書きできる。

> **注記（Informative）— 例外**: 役割をひとつに割り当てられない塊の第三者 CSS（フォームプラグインや CSS フレームワーク等、丸ごと取り込んだ上で自作スタイルにより上書きしたいもの）は、専用の追加層（慣用名 `vendor`）を project が宣言してよい。本追加層を foundation より下位（先制宣言で foundation より前に宣言される位置）に置くことで、自作の foundation・component・project 等の層の通常宣言が当該第三者 CSS の通常宣言に優先される。ただし第三者 CSS が内部で `!important` を使用している場合（「例外: `!important` の許容範囲」類型 2）、当該宣言は重要度により自作層の通常宣言に優先され、Utility 層の `!important` に対しても層順序の逆転により優先される。本追加層は不変原則 #4「CSS の進化への追従」（§1.2）が認める層構成の適応的な設計余地に基づく project 固有の拡張であり、§3 の層構成には加えない。役割で畳める機能ライブラリを本層に入れることはその用途を薄め、上記原則の意図しない上書きリスクを生むため、project が意図的に最弱隔離を選ぶ場合を除き避ける。
...
3. **最終上書きの保証**: `!important` による、層の外（unlayered CSS・インラインスタイル）を含むすべての通常宣言に対する優先
...
> **注記（Informative）**: MUST [!important の付与] が宣言を選別せず全宣言を対象とするのは、責任 3 の最終上書きが §2 準拠条件にいう「他ルール違反時のフェイルセーフ」であることに加え、準拠状態でも残りうる層の外の通常宣言に対する保証を兼ねるからである。§3 の標準層構成に従う限り、Utility の通常宣言は層順序（§3）だけで他層の通常宣言に優先する。加えて、§4 MUST [外部 CSS の層取り込み] により外部 CSS 由来の unlayered CSS が、§7 MUST NOT [静的インラインスタイルの禁止] により静的インラインスタイルが排除されるため、準拠プロジェクトで層の外に残る通常宣言は限られる。それでも `!important` が効果を持つ場面は残る — これらのルールが破られた場合、自作スタイルが `@layer` の外に置かれた場合（自作スタイルの層内記述を義務づける MUST は本仕様に無い）、および §7 が対象外とする JS による動的なスタイル注入や CMS・ライブラリが自動生成するインラインスタイルに対して等である。後二者は準拠プロジェクトでも通常起こりうる。どの Utility クラスのどの宣言がそうした層の外の通常宣言と衝突するかは事前に確定できないため、宣言単位で選別せず全宣言に付与する。付与を欠いた宣言は、§3 の標準層構成において他層の通常宣言には優先するが、層の外の通常宣言には優先しない。本保証は通常宣言に対するものであり、`!important` 宣言同士の優先関係は §3 注記および §4 [外部 CSS の層配置判断] の例外注記を参照。
...
> **注記（Informative）**: インラインスタイル（要素の `style` 属性に記述された宣言）は、カスケードにおいて層への割り当て（Layers）より先に評価される独立した判定軸（Element-Attached Styles [CSS-CASCADE-5]）に属し、`@layer` ブロック外に記述されたスタイル（unlayered CSS）とは別のカテゴリである。その通常宣言は全ての layered CSS の通常宣言より優先され、`!important` を伴う場合は layered の `!important` 宣言にも優先されるため、層構造による優先順位制御を破壊する。
...
| **通常宣言** | `!important` を伴わない宣言（normal declaration [CSS-CASCADE-5]）。`@layer` の層順序は、通常宣言と `!important` 宣言で適用方向が逆になる（初出: §3） |
...
| **unlayered CSS** | `@layer` ブロック外に記述されたスタイル。通常宣言は全ての layered CSS の通常宣言に優先するが、`!important` 宣言では層順序の逆転により layered の `!important` 宣言に優先されない。インラインスタイル（要素の `style` 属性に記述された宣言）は本用語に含まない — 層への割り当てより先に評価される別の判定軸（Element-Attached Styles）に属し、`!important` 同士でも layered CSS に優先する（初出: §7） |
