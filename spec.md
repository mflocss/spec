...
- 層の追加・削除・統合、または既存の MUST/MUST NOT の変更はメジャーバージョン変更とする
- MUST/MUST NOT が関わる要求レベルの変更（昇格・降格・新設・削除）はメジャーバージョン変更とする。SHOULD/MAY の追加・削除・レベル間の変更はマイナーバージョン変更とする
- 新規ルールの追加（SHOULD/MAY）、Informative セクションの変更はマイナーバージョン変更とする
- 誤記修正・Example の改善等、要求レベルに影響しない変更はパッチバージョン変更とする
...
- SHOULD NOT [クラスセレクタ・ID セレクタの回避]: クラスセレクタ・ID セレクタを使用すべきでない。Foundation 層は要素型セレクタを優先して使用し、全ページ共通の基本スタイルを定義する
...
カスタムプロパティのトークン参照チェーン（Token 層内のプリミティブ → セマンティックの参照パスと、Foundation 以降の層によるセマンティック変数の参照ルール）、およびインラインスタイルの使用制限を定義する。グローバルトークンは Foundation 以降の層から直接参照してよい（MAY）。
