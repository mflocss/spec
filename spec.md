...
> **注記（Informative）**: インラインスタイルはどの `@layer` にも属さない unlayered CSS であり、カスケードでは層への割り当てより先に評価される。その通常宣言は全ての layered CSS の通常宣言より優先され、`!important` を伴う場合は layered の `!important` 宣言にも優先されるため、層構造による優先順位制御を破壊する。
...
| **unlayered CSS** | `@layer` ブロック外に記述されたスタイル。通常宣言は全ての layered CSS の通常宣言に優先するが、`!important` 宣言では層順序の逆転により layered の `!important` 宣言に優先されない。ただしインラインスタイル（要素に直接付与された宣言）は層への割り当てより先に評価されるため、`!important` 同士でも layered CSS に優先する（初出: §7） |
