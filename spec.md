...
Step 2: ブラウザデフォルトの正規化（リセット CSS）か？
  └─ Yes → Reset
...
Step 5: 別のサイトにそのまま持っていけるか？
  ├─ Yes → Component
  └─ No → Project
  ※ 判断が曖昧な場合は、§5.5 の補助テスト（Responsibility Test）を使用する。

※ Step 6-7 は Step 1-5 の判定結果とは独立して評価する。
  1つのスタイルが Step 5 で Component と判定され、かつ Step 6 で
