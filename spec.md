...
> **注記（Informative）**: project が §3 の標準層構成に加えて宣言する追加層（§3 注記および §4 [外部 CSS の層配置判断] の例外注記が挙げる慣用名 `vendor` 等）も「`@layer` の各層定義」に含まれ、本仕様の要求レベルは追加層にも及ぶ。特に §4 MUST NOT [!important の使用制限] の「Utility 層を除く全層」は追加層を含む。外部生成クラスに対する前段の免除は、そのクラスをセレクタに用いること、およびそのクラス名が §6 の命名規則に従わないことについてのものであり、project が記述する宣言の内容（`!important` の付与を含む）を免除しない。したがって、追加層に取り込んだ第三者 CSS の `!important` 宣言（§4「例外: `!important` の許容範囲」類型 2）を project が自作の `!important` 宣言で打ち消すことは MUST NOT [!important の使用制限] に違反し、本仕様はその手段を提供しない。やむを得ず行う場合、当該プロジェクトは本仕様への準拠を主張できず、逸脱の箇所と理由をコード内コメントまたは設計文書に残すことを推奨する。
...
> **注記（Informative）**: MUST [!important の付与] が宣言を選別せず全宣言を対象とするのは、責任 3 の最終上書きが §2 準拠条件にいう「他ルール違反時のフェイルセーフ」だからである。準拠プロジェクトでは §4 MUST [外部 CSS の層取り込み] により unlayered CSS が、§7 MUST NOT [静的インラインスタイルの禁止] により静的インラインスタイルが排除されるため、Utility の通常宣言は層順序（§3）だけで他層の通常宣言に優先する。`!important` が効果を持つのは、これらのルールが破られた場合、および §7 が対象外とする JS による動的なスタイル注入や CMS・ライブラリが自動生成するインラインスタイルに対してであり、後者は準拠プロジェクトでも通常起こりうる。どの Utility クラスのどの宣言がそうした層外の通常宣言と衝突するかは事前に確定できないため、宣言単位で選別せず全宣言に付与する。付与を欠いた宣言は他層の通常宣言には優先するが、層外の通常宣言には優先されない。本保証は通常宣言に対するものであり、`!important` 宣言同士の優先関係は §3 注記および §4 [外部 CSS の層配置判断] の例外注記を参照。
