...
> **注記（Informative）**: `@layer` の先制宣言では、通常宣言について後に宣言した層ほど優先されるため、テーブルの下にある層ほど優先度が高い。`!important` 宣言では層順序が逆転する（先に宣言した層が優先される）ため、Utility の最終上書きは宣言順ではなく、§5.8 MUST [!important の付与] と §4 MUST NOT [!important の使用制限] の組で成立する。
...
> **注記（Informative）**: インラインスタイルはどの `@layer` にも属さない unlayered CSS であり、カスケードでは層への割り当てより先に評価される。その通常宣言は全ての layered CSS の通常宣言より優先されるため、層構造による優先順位制御を破壊する。
...
| **unlayered CSS** | `@layer` ブロック外に記述されたスタイル。通常宣言同士では全ての layered CSS に優先する。`!important` 宣言では層順序が逆転するため、layered の `!important` 宣言には優先されない（初出: §7） |
