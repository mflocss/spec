...
- MAY [プラットフォーム不変条件の保護]: ユーザーエージェントスタイルシート [HTML] が要素の特定の状態に対して `display: none` を定める表示上の不変条件を、後続層の宣言による意図しない上書きから保護する目的に限り、自作の宣言に `display: none !important` を付与してよい。保護する宣言は、ユーザーエージェントスタイルシートが当該不変条件を適用する状態と同一の状態（例: `[hidden]`（`until-found` 値を除く）、`dialog:not([open])`、`[popover]:not(:popover-open)`）に限定しなければならず、要素のその他の状態の表示を上書きしてはならない。ユーザーエージェントスタイルシートが `display: none` 以外の宣言を定める場合（例: `[hidden=until-found]` の `content-visibility`）は本許容の対象外とする。
...
3. **プラットフォーム不変条件の保護**: MAY [プラットフォーム不変条件の保護] に基づき、ユーザーエージェントスタイルシート [HTML] が定める `display: none` の表示不変条件を保護する場合に限り、自作の `display: none !important` を許容する。
