...
> **注記（Informative）**: 先制宣言で確定した層順序では、通常宣言について後に宣言された層ほど優先されるため、テーブルの下にある層ほど優先度が高い。`!important` 宣言では層順序が逆転する（先に宣言した層が優先される）ため、Utility の最終上書きは宣言順ではなく、§5.8 MUST [!important の付与] と §4 MUST NOT [!important の使用制限] の組で成立する。§4 の例外により他層が `!important` を持つ場合は、層順序の逆転によりその宣言が Utility に優先される。
...
| **通常宣言** | `!important` を伴わない宣言 [CSS-CASCADE-5]。`@layer` の層順序は、通常宣言と `!important` 宣言で適用方向が逆になる（初出: §3） |
...
| **unlayered CSS** | `@layer` ブロック外に記述されたスタイル。通常宣言では全ての layered CSS に優先するが、`!important` 宣言では層順序の逆転により layered の `!important` 宣言に優先されない。ただしインラインスタイル（要素に直接付与された宣言）は層への割り当てより先に評価されるため、`!important` 同士でも layered CSS に優先する（初出: §7） |
