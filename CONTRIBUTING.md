...
3. 手順 1・2 の変更をコミットし、`main` にマージする
4. `main` で `git tag vX.Y.Z` を切り、`git push origin vX.Y.Z` でリモートに反映する
5. GitHub Release を作成する（本文は当該バージョンの CHANGELOG から起こす）
6. 次バージョンの変更を記録し始める際に、新しい `## [vX.Y.Z] - Unreleased` セクションを CHANGELOG 上部に追加
