...
- CHANGELOG は、前回リリース以降の変更をリリース対象バージョンの `## [vX.Y] - Unreleased` セクションに追記していきます。
- 初版（v1.0）は比較対象の前バージョンが存在しないため変更履歴を持たず、`初版リリース。` のみとします。

## リリース手順

1. リリースするバージョンの `## [vX.Y] - Unreleased` 見出しを `## [vX.Y] - YYYY-MM-DD` に置換（リリース実日付を記入）
2. `git tag vX.Y` でリリースタグを切る
3. 次バージョンの変更を記録し始める際に、新しい `## [vX.Y] - Unreleased` セクションを CHANGELOG 上部に追加
